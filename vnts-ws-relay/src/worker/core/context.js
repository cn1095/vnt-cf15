...
class ExpireMap {
  ...
  set(key, value, customTtlMs = null) {
    ...
    logger.debug(
      `[过期映射-设置] 键: ${key}, TTL: ${ttl}ms, 过期时间: ${new Date(
        expireTime
      ).toISOString()}`
    );
  ...
  cleanup() {
    ...
    for (const [key, item] of this.map.entries()) {
      if (now > item.expireTime) {
        logger.debug(
          `[过期映射-清理] 删除过期项: ${key} (过期于: ${new Date(
            item.expireTime
          ).toISOString()})`
        );
        this.map.delete(key);
  ...
  destroy() {
    if (this.cleanupTimer) {
      ...
      this.map.clear();
      logger.debug(
        `[过期映射-销毁] 已清理 ${itemCount} 个剩余项，ExpireMap已完全销毁`
      );
    }
  }
}
...
export class VntContext {
  ...
  async leave(cache) {
    ...
    if (this.link_context) {
      ...
      if (networkInfo) {
        logger.debug(
          `[连接清理-网络] 找到网络信息: ${this.link_context.group}`
        );
        ...
        if (clientInfo) {
          logger.debug(
            `[连接清理-客户端] 找到客户端信息: ${this.link_context.virtual_ip}`
          );
          // 验证地址和时间戳匹配
          if (
          ...
          // 更新客户端状态
          logger.debug(
            `[连接清理-状态] 更新客户端状态为离线: ${this.link_context.virtual_ip}`
          );
